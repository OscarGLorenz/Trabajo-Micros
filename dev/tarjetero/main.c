...
void tanimoto() {
  ...
  for (int j = 0; j < n; j++) {
    ...
    serialPrintInt(jaccard[j]);
    serialPrint("\t");
  }
  flanco_actual = 0;
  if ((jaccard[whoMax(jaccard)] >= 3)) { //Si hay 1 letra mayor que D, descartamos mucho malo. Si no, todo bien. Sin assembly 0.75, con assembly, 3
